...
StaticTheme::StaticTheme(PixelMatrix &matrix) : Animation(matrix) {
    ...
    const std::map<uint32_t, RGB> theme2XKO({
        // Direction
        {GAMEPAD_MASK_DU, RGB(241, 241, 241)},
        {GAMEPAD_MASK_DD, ColorWhite},
        {GAMEPAD_MASK_DL, ColorWhite},
        {GAMEPAD_MASK_DR, ColorWhite},

        // Team
        {GAMEPAD_MASK_B1, RGB(191, 245, 68)},

        // Attack
        {GAMEPAD_MASK_B2, ColorPurple},
        {GAMEPAD_MASK_B3, ColorPurple},
        {GAMEPAD_MASK_B4, ColorPurple},

        // Supers
        {GAMEPAD_MASK_R2, RGB(255, 40, 53)},
        {GAMEPAD_MASK_L2, RGB(0, 200, 252)},

        // Other macros
        {GAMEPAD_MASK_R3, ColorRed},
        {GAMEPAD_MASK_R1, ColorOrange},
        {GAMEPAD_MASK_L1, ColorBlue},
    });
            ...
            for (size_t c = 0; c != matrix->pixels[r].size(); c++) {
                ...
                if (itr != theme.end()) {
                    for (size_t p = 0; p != matrix->pixels[r][c].positions.size(); p++) {
                        // Interpolate from hitColor (color the button was assigned when pressed) back to the theme
                        // color
                        frame[matrix->pixels[r][c].positions[p]] = BlendColor(
                            hitColor[matrix->pixels[r][c].index], itr->second, times[matrix->pixels[r][c].index]);
                    }
                } else {
                    for (size_t p = 0; p != matrix->pixels[r][c].positions.size(); p++) {
