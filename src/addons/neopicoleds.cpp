...
const RGB alCustomStaticColors[AL_STATIC_COLOR_COUNT]{ColorBlack,  ColorWhite,     ColorRed,   ColorOrange,
                                                      ColorYellow, ColorLimeGreen, ColorGreen, ColorSeafoam,
                                                      ColorAqua,   ColorSkyBlue,   ColorBlue,  ColorPurple,
...
PLEDAnimationState getXBoneAnimationNEOPICO(Gamepad* gamepad) {
    PLEDAnimationState animationState = {
        .state = (PLED_STATE_LED1 | PLED_STATE_LED2 | PLED_STATE_LED3 | PLED_STATE_LED4), .animation = PLED_ANIM_OFF
    };
...
PLEDAnimationState getPS3AnimationNEOPICO(uint16_t ledState) {
    const uint8_t ps3LEDs[10][4] = {{0x01, 0x00, 0x00, 0x00}, {0x00, 0x01, 0x00, 0x00}, {0x00, 0x00, 0x01, 0x00},
                                    {0x00, 0x00, 0x00, 0x01}, {0x01, 0x00, 0x00, 0x01}, {0x00, 0x01, 0x00, 0x01},
    ...
    if (ledState != 0) {
        ...
        if (ps3LEDs[ledNumber - 1][0] == 0x01) {
            animationState.state |= PLED_STATE_LED1;
        }
        if (ps3LEDs[ledNumber - 1][1] == 0x01) {
            animationState.state |= PLED_STATE_LED2;
        }
        if (ps3LEDs[ledNumber - 1][2] == 0x01) {
            animationState.state |= PLED_STATE_LED3;
        }
        if (ps3LEDs[ledNumber - 1][3] == 0x01) {
            animationState.state |= PLED_STATE_LED4;
        }
    }
    ...
    switch (options.ambientLightEffectsCountIndex) {
        case AL_CUSTOM_EFFECT_STATIC_COLOR:
            for (int i = 0; i < maxFrame; i++) {
                frame[alStartIndex + i] = alCustomStaticColors[options.alCustomStaticColorIndex].value(
                    Animation::format, options.alStaticColorBrightnessCustomX);
            }
            break;

        case AL_CUSTOM_EFFECT_GRADIENT:
            alFrameToRGB = 255 - alCurrentFrame; // From 0 -> 255 to 255 -> 0
            if (alFrameToRGB < 85) { // Less than 85, transitions from red to yellow. The red component starts at 255
                                     // and gradually decreases, the green component always reaches 0, and the blue
                                     // component starts at 0 and increases gradually.
                ambientLight.r = 255 - alFrameToRGB * 3;
                ambientLight.g = 0;
                ambientLight.b = alFrameToRGB * 3;
            } else if (alFrameToRGB < 170) { // Between 85 and 170, there is a transition from yellow to cyan. The red
                                             // component is always 0, the green component starts at 0 and gradually
                                             // increases, and the blue component starts at 255 and gradually decreases.
                alFrameToRGB -= 85;
                ambientLight.r = 0;
                ambientLight.g = alFrameToRGB * 3;
                ambientLight.b = 255 - alFrameToRGB * 3;
            } else { // When greater than or equal to 170, it transitions from cyan to blue. The red component is always
                     // 0, the green component starts at 255 and gradually decreases, and the blue component starts at 0
        ...
        case AL_CUSTOM_EFFECT_CHASE:
            if (time_reached(nextRunTimeAmbientLight)) {
                ...
                if (alFrameToRGB < 85) { // Less than 85, transitions from red to yellow. The red component starts at
                                         // 255 and gradually decreases, the green component always reaches 0, and the
                                         // blue component starts at 0 and increases gradually.
                    ambientLight.r = 255 - alFrameToRGB * 3;
                    ambientLight.g = 0;
                    ambientLight.b = alFrameToRGB * 3;
                } else if (alFrameToRGB <
                           170) { // Between 85 and 170, there is a transition from yellow to cyan. The red component is
                                  // always 0, the green component starts at 0 and gradually increases, and the blue
                                  // component starts at 255 and gradually decreases.
                    alFrameToRGB -= 85;
                    ambientLight.r = 0;
                    ambientLight.g = alFrameToRGB * 3;
                    ambientLight.b = 255 - alFrameToRGB * 3;
                } else { // When greater than or equal to 170, it transitions from cyan to blue. The red component is
                         // always 0, the green component starts at 255 and gradually decreases, and the blue component
                         // starts at 0 and gradually increases.
                    alFrameToRGB -= 170;
                    ambientLight.r = alFrameToRGB * 3;
        ...
        case AL_CUSTOM_EFFECT_STATIC_THEME:
            ...
            for (int i = 0; i < multipleOfCustomStaticThemeCount; i++) {
                for (int j = 0; j < AL_COL; j++) {
                    frame[alStartIndex + i * AL_COL + j] =
                        alCustomStaticTheme[options.alCustomStaticThemeIndex][j].value(
                            Animation::format, options.alStaticBrightnessCustomThemeX);
                }
            }
            if (remainderOfCustomStaticThemeCount != 0) {
                for (int k = 0; k < remainderOfCustomStaticThemeCount; k++) {
                    frame[alStartIndex + multipleOfCustomStaticThemeCount * AL_COL + k] =
                        alCustomStaticTheme[options.alCustomStaticThemeIndex][k].value(
                            Animation::format, options.alStaticBrightnessCustomThemeX);
                }
            }
            break;
...
void NeoPicoLEDAddon::ambientLightLinkage() {
    ...
    for (int i = 0; i < multipleOfButtonLedsCount; i++) { // Repeat buttons
        for (int j = 0; j < buttonLedCount; j++) {
            frame[alLinkageStartIndex + i * buttonLedCount + j] =
                as.linkageFrame[j].value(Animation::format, preLinkageBrightnessX);
        }
    }

    if (remainderOfButtonLedsCount != 0) { // Remainder
        for (int k = 0; k < remainderOfButtonLedsCount; k++) {
            frame[alLinkageStartIndex + multipleOfButtonLedsCount * buttonLedCount + k] =
                as.linkageFrame[k].value(Animation::format, preLinkageBrightnessX);
        }
    }
}
...
void NeoPicoLEDAddon::process() {
        ...
        if (gamepad->auxState.playerID.enabled && gamepad->auxState.playerID.active) {
            switch (gamepad->getOptions().inputMode) {
                ...
                case INPUT_MODE_P5GENERAL:
                    animationState = getPS4AnimationNEOPICO(gamepad->auxState.playerID.ledBlinkOn,
                                                            gamepad->auxState.playerID.ledBlinkOff);
                    break;
                case INPUT_MODE_XBONE: animationState = getXBoneAnimationNEOPICO(gamepad); break;
    ...
    if (ledOptions.pledType == PLED_TYPE_RGB) {
        int32_t pledIndexes[] = {
            ledOptions.pledIndex1, ledOptions.pledIndex2, ledOptions.pledIndex3, ledOptions.pledIndex4
        };
        for (int i = 0; i < PLED_COUNT; i++) {
            ...
            float level =
                (static_cast<float>(PLED_MAX_LEVEL - neoPLEDs->getLedLevels()[i]) / static_cast<float>(PLED_MAX_LEVEL));
            ...
            if (gamepad->auxState.sensors.statusLight.enabled && gamepad->auxState.sensors.statusLight.active) {
                rgbPLEDValues[i] = (RGB(gamepad->auxState.sensors.statusLight.color.red,
                                        gamepad->auxState.sensors.statusLight.color.green,
                                        gamepad->auxState.sensors.statusLight.color.blue))
                                       .value(neopico.GetFormat(), brightness);
            } else {
    ...
    if (turboOptions.turboLedType == PLED_TYPE_RGB) {                                  // RGB or PWM?
        if (gamepad->auxState.turbo.activity == 1) {                                   // Turbo is on (active sensor)
            if (turboOptions.turboLedIndex >= 0 && turboOptions.turboLedIndex < 100) { // Double check index value
                ...
                frame[turboOptions.turboLedIndex] =
                    ((RGB)turboOptions.turboLedColor).value(neopico.GetFormat(), brightness);
            }
        }
    }
...
#define PIXEL(BUTTON, MASK) Pixel(buttonPositions[BUTTON], MASK, *getLEDPositions(BUTTON, positions))
...
std::vector<std::vector<Pixel>> NeoPicoLEDAddon::createLEDLayout(ButtonLayout layout, uint8_t ledsPerPixel,
    ...
    switch (static_cast<ButtonLayout>(layout)) {
        ...
        case BUTTON_LAYOUT_BOARD_DEFINED_A    : return generatedLEDStickless(&positions);
        case BUTTON_LAYOUT_FIGHTBOARD_MIRRORED: return generatedLEDWasdFBM(&positions);
        case BUTTON_LAYOUT_BUTTONS_ANGLED     :
        case BUTTON_LAYOUT_FIGHTBOARD_STICK   : return generatedLEDWasd(&positions);
        ...
        default                               : return generatedLEDButtons(&positions);
    }
        ...
        if (gamepad->pressedL1()) { // LB
            ...
            if (lastAmbientAction != action) {
                ...
                if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_CHASE) {
                    animationOptions.alChaseBrightnessCustomX =
                        min(animationOptions.alChaseBrightnessCustomX + customBrightnessStep, 1.00f);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_GRADIENT) {
                    animationOptions.alGradientBrightnessCustomX =
                        min(animationOptions.alGradientBrightnessCustomX + customBrightnessStep, 1.00f);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_STATIC_COLOR) {
                    animationOptions.alStaticColorBrightnessCustomX =
                        min(animationOptions.alStaticColorBrightnessCustomX + customBrightnessStep, 1.00f);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_STATIC_THEME) {
                    animationOptions.alStaticBrightnessCustomThemeX =
                        min(animationOptions.alStaticBrightnessCustomThemeX + customBrightnessStep, 1.00f);
            ...
            if (lastAmbientAction != action) {
                ...
                if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_CHASE) {
                    animationOptions.alChaseBrightnessCustomX =
                        max(animationOptions.alChaseBrightnessCustomX - customBrightnessStep, 0.00f);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_GRADIENT) {
                    animationOptions.alGradientBrightnessCustomX =
                        max(animationOptions.alGradientBrightnessCustomX - customBrightnessStep, 0.00f);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_STATIC_COLOR) {
                    animationOptions.alStaticColorBrightnessCustomX =
                        max(animationOptions.alStaticColorBrightnessCustomX - customBrightnessStep, 0.00f);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_STATIC_THEME) {
                    animationOptions.alStaticBrightnessCustomThemeX =
                        max(animationOptions.alStaticBrightnessCustomThemeX - customBrightnessStep, 0.00f);
                } // do nothing for breath
                reqSave = true;
            }
            ...
            if (lastAmbientAction != action) {
                if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_GRADIENT) {
                    animationOptions.ambientLightGradientSpeed =
                        min(animationOptions.ambientLightGradientSpeed + 1, (uint32_t)6);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_CHASE) {
                    animationOptions.ambientLightChaseSpeed =
                        max(animationOptions.ambientLightChaseSpeed - 20, (int32_t)0);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_BREATH) {
                    animationOptions.ambientLightBreathSpeed =
                        min(animationOptions.ambientLightBreathSpeed + 0.01f, 0.05f);
                }
                reqSave = true;
            }
            ...
            if (lastAmbientAction != action) {
                if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_GRADIENT) {
                    animationOptions.ambientLightGradientSpeed =
                        max(animationOptions.ambientLightGradientSpeed - 1, (uint32_t)1);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_CHASE) {
                    animationOptions.ambientLightChaseSpeed =
                        min(animationOptions.ambientLightChaseSpeed + 20, (int32_t)100);
                } else if (animationOptions.ambientLightEffectsCountIndex == AL_CUSTOM_EFFECT_BREATH) {
                    animationOptions.ambientLightBreathSpeed =
                        max(animationOptions.ambientLightBreathSpeed - 0.01f, 0.01f);
                }
                reqSave = true;
            }
